...
dy(1) = (r*(-g_ampa*s_e - g_gabaa*s_i &
     & + k*(2.0*v - v_r - v_t)) + Delta*k**2*abs(v-v_r)/(pi*C))/C
dy(2) = (pi*C*r*(Delta*sign(dble(1), v-v_r) - pi*C*r/k) &
     & + I_ext + g_ampa*s_e*(E_ampa - v) + g_gabaa&
     & *s_i*(E_gabaa - v) + k*v*(v - v_r - v_t) + k*v_r*v_t - u)/C
dy(3) = a*(b*(v - v_r) - u) + d*r
dy(4) = r - s/tau_ampa
dy(5) = (r_0*(-g_ampa*s_e_0 - g_gabaa*s_i_0 &
     & + k_0*(2.0*v_0 - v_r_0 - v_t_0)) + Delta_0*k_0**2*abs(v_0-v_r_0)&
     & /(pi*C_0))/C_0
dy(6) = (pi*C_0*r_0*(Delta_0**sign(dble(1), v_0-v_r_0) - pi*C_0*r_0/k_0)&
     & + I_ext_0 + g_ampa*s_e_0*(E_ampa - v_0) + g_gabaa&
     & *s_i_0*(E_gabaa - v_0) &
